...
- F-04.05：给角色上传参考图。浏览器经短时效签名 URL **直传对象存储**（§26），所以一次请求预留存储键并签发写权限，第二次确认真的到了什么。中间没有任何一步被信任：签名把声明的类型与字节数**签进去**（`content-length` 脚本不能设，浏览器填真实体积，于是签名等于把请求体钉死在那个长度），确认时读回字节验魔数 + 长度 + SHA-256 才让 Asset 变成 `ready`。
  ...
  - F-04.06 把上传交互抽成 `reference-image-field.tsx`，创建表单也有了入口 —— 在那之前版本 1 永远不可能带图，而用版本 1 出的片就没有参考图可看。组件是受控的：ids 由提交的表单持有，权利勾选、上传、预览 URL 的生命周期由组件自己管。
  - **尚未做**：孤立对象的 Reconciler 与保留周期（ADR-006 已记）。
...
- 参考图的上传交互在 `apps/web/components/reference-image-field.tsx`，创建与编辑两个表单共用它 —— 「先上传确认 Asset、再写引用它的版本」这个顺序不显然，两份实现就是两次弄错的机会。
