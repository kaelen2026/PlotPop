...
字段级消息与表单级摘要**同时**存在，所以同一句话会出现两次 —— 这是要求，不是重复。但**只能有一个 Live Region**：注册表的 `FieldError` 默认自带 `role="alert"`，两个字段出错就会播报三次。有摘要的表单给 `FieldError` 传 `announce={false}`，由摘要独家承担播报。

字段级消息必须通过 `aria-describedby` 挂到控件上，否则它只是屏幕上挨着输入框的一段文字，屏幕阅读器读不到。

提交被拒时焦点移到摘要（摘要带 `tabIndex={-1}`）。把焦点留在一个「按了没反应」的提交按钮上，是键盘用户卡住的典型原因。

Zod Schema 里**不写用户可见文案**：§14 要求文案来自本地化资源，所以 Schema 只描述约束，由表单按 `issue.path` 与 `issue.code` 映射到本地化句子。长度上限这类数字由 Schema 导出常量、文案插值引用，避免文案承诺一个 Schema 并不强制的限制。
...
余额是**服务端给出的字段**，组件不接收账本条目 —— §10 规定客户端不得计算权威余额。把给定的余额与给定的预估相比是展示逻辑，从账本推导余额不是。

「余额是否足够」按预估的**上界**判断，不是下界。区间下界放得下、上界放不下的生成不算可负担；启动一个注定跑不完的生成比不启动更糟。

「是否需要再次确认」由服务端声明（`changeReason`），不由客户端推断：ADR-005 要求用户确认过的价格上限不得被自动路由突破。变更原因是**产品级枚举**，不含供应商身份（不变量 4）。

**实现状态**：`packages/ui` 的 `CreditCost` 组件，向导动画步骤在用。真实报价由服务端产生，属 F-06。
