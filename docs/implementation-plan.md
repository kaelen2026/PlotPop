...
- 类型检查、Lint 和相关测试通过。
- 所有行为变更遵循 TDD，提交中必须能识别 Red、Green、Refactor 过程。
...
1. **Specify：**将任务拆成一个可以通过外部行为描述的最小增量，明确输入、输出、失败和边界。
2. **Red：**先添加或修改测试，运行并确认测试因目标行为尚未实现而失败。
3. **Green：**编写满足当前测试的最小实现，不提前实现后续需求。
4. **Refactor：**在测试保持通过的前提下消除重复、改善命名和边界。
5. **Verify：**运行受影响包测试、类型检查、Biome Check 和必要的集成测试。
6. **Commit：**提交一个可独立解释和回退的行为增量。

禁止：

- 先完成实现再补测试。
- 为让测试通过而删除断言或降低断言强度。
- 在同一个 Red 阶段同时引入多个不相关行为。
- 通过 Mock 内部实现细节来替代用户可观察行为。
- 将真实模型、支付或对象存储作为 CI 测试的必要条件。
- 在测试未通过时开始下一个任务。
...
技术 Spike 不能直接合并为生产实现。验证结束后必须删除或隔离 Spike，并按 TDD 重新实现正式代码。

### 4.2 测试层级选择

每个行为优先使用成本最低且能稳定证明需求的测试：

- Domain：纯函数单元测试。
- Schema：Zod 有效、无效和边界输入测试。
- Database：真实 PostgreSQL 集成测试，不 Mock Drizzle 行为。
- API：Hono `testClient()` 路由测试，覆盖鉴权、状态码与 RPC 类型。
- Worker：Fake Provider、真实 Redis/PostgreSQL 测试容器和可控时钟。
- Web Component：Testing Library，以角色、名称和用户操作查询。
- End-to-End：Playwright 覆盖关键用户旅程，不重复穷举底层规则。
- Visual：Light、Dark 和关键响应式断点的视觉回归。

测试不得依赖执行顺序。时间、随机数、外部 ID 和供应商响应必须可控。

### 4.3 垂直任务的 TDD 模板

每个 F-xx 垂直任务继续拆成若干 `F-xx.yy` 行为任务。每个行为任务必须记录：
...
一个行为任务建议在一次短开发循环内完成。任务过大时，按成功路径、校验失败、权限失败、并发冲突和恢复路径继续纵向拆分。
...
后续第 5–16 节是技术能力检查清单，不代表严格执行顺序。实际执行顺序以第 4.5–4.8 节的任务依赖图为准。
...
**移交给 F-11 的两项（决定于 F-02 收尾时）：**

- 镜头检查器的编辑表单（角色、提示词、台词、声音、机位、运动、时长、模型档位）。
- 小屏的审阅与批准布局。

理由：这两项要操作的对象在 F-02 阶段还不存在。检查器的字段契约属 F-04（角色与资产）与 F-05（场景与镜头），「局部重生成」和「批准」的真实语义依赖 Outbox（ADR-003）、积分预留（ADR-004）与 Shot Version（ADR-006）。在这些契约之前把表单按占位数据做出来，交付的是一套需要重写的字段，而不是可验证的行为 —— 而 F-02 的目的是让创作者验证**流程**，流程已经可以走通。

F-11 的范围本来就包含「Episode Studio 三栏工作台」，所以这不是新增工作，是把它放回依赖就绪的位置。
...
**实际交付（F-02 已收尾）：**设计 Token 与三态主题（首绘前解析）、`packages/ui` 与 shadcn/ui 接入、Creator Home（空态与列表）、统一状态表达、五步向导（脚本步骤含 Zod 校验）、生成前的积分预估与确认、Episode Studio 三栏工作台（浏览与时间线）、英文本地化资源、Light/Dark 视觉基线与全页面 axe 审计。四道门禁随之建立：对比度验证、业务代码视觉硬编码扫描、基础组件选中态表达、Light/Dark 视觉回归。
...
**已完成：**签名上传、上传确认、参考图钉到 Character Version，以及归属、格式与重复确认测试。**Media Worker 的文件验证仍未做** —— 完整性检查（真实 MIME、字节数、SHA-256）由 API 在确认请求内同步完成，媒体参数与安全扫描等 F-05 的 Outbox 落地后交给 Worker。分工与理由见 `docs/ai-comic-drama-saas-design.md` §26.1。

**仍未做：**创建角色的表单还没有上传入口（先用文字建角色、再编辑加图）；孤立对象的 Reconciler 与各类素材的保留周期（ADR-006 的已知代价）；系列的声音、风格手册与默认生成设置。
...
- **镜头检查器的编辑表单**（角色、提示词、台词、声音、机位、运动、时长、模型档位）与局部重生成入口 —— 自 F-02 移交，理由见 §4.5 F-02。字段契约来自 F-04 与 F-05，动作语义来自 ADR-003、ADR-004 与 ADR-006。
- **小屏的审阅与批准布局** —— 自 F-02 移交。批准要操作的 Shot Version 来自 F-07。
...
- 每个并行任务独立执行完整 Red → Green → Refactor，不共享另一个分支尚未合并的测试夹具。
- 共享测试工具必须先作为独立 TDD 增量合并到 `packages/testkit`。
...
- 按 `docs/research/consistency-gate.md` 的评分表与盲评规程组织人工评分。该文件必须在第一次生成前冻结。
- 计算每镜头与每成片分钟的 P50、P95 成本和耗时。
- 记录供应商审核、限流和失败行为。

### 5.3 产物

- `docs/research/consistency-gate.md`（门禁定义，第一次生成前冻结）
...
- 已按 `docs/research/consistency-gate.md` 的四条门禁判断角色一致性是否足以进入 MVP，并完成 Gate A 的经济复核。
...
- CI 能检测空测试、被 `.only` 或 `.skip` 意外排除的测试，以及未处理的 Promise Rejection。
- 测试命令支持按包、按文件和全仓库执行，便于短 TDD 循环。
...
1. F-00.01：先定义供应商评估结果 Schema 与失败测试，再实现结果记录器。
2. F-00.02：先定义一致性评分统计测试，再实现统计逻辑。
3. F-01.01：建立只包含失败健康检查测试的 Web、API、Worker 骨架，再实现健康检查。
4. F-01.02：为共享配置和环境变量先编写 Zod Schema 边界测试，再实现配置包。
5. F-01.03：配置 TypeScript、Biome、Husky、lint-staged、commitlint、Vitest 和 CI。
6. F-01.04：先编写容器健康验证，再建立 Docker Compose 本地环境。
7. F-02.01：先建立 Light、Dark Token 快照和禁止硬编码检查，再实现主题 Token。
8. F-02.02：用组件交互与可访问性测试驱动 Creator Home、向导和 Episode Studio 静态原型。
